...
use fs_wasm::{
    fs_close, fs_fstat, fs_mkdir, fs_open_path, fs_open_path_with_flags, fs_read, fs_seek,
    fs_write, FsStat, O_APPEND, O_CREAT, O_RDONLY, O_RDWR, O_TRUNC, O_WRONLY,
};
...
fn demo_append_operations() -> Result<(), String> {
    unsafe {
        ...
        if content != "Initial First Second Third" {
            fs_close(fd as u32);
            return Err(format!(
                "Content mismatch: expected 'Initial First Second Third', got '{}'",
                content
            ));
        }
...
fn demo_trunc_operations() -> Result<(), String> {
    unsafe {
        ...
        fs_close(fd as u32);
        println!(
            "Created file with old content ({} bytes)",
            old_content.len()
        );
        ...
        if read_bytes != 0 {
            fs_close(fd as u32);
            return Err(format!(
                "Expected empty file, but read {} bytes",
                read_bytes
            ));
        }
