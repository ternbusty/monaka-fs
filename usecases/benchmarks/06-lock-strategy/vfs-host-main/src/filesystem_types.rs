...
//
// Implements wasi:filesystem/types interface using main-branch fs-core with Mutex
...
pub struct FsInputStreamWrapper {
    /// Reference to shared VFS (with external Mutex)
    shared_vfs: Arc<Mutex<Fs>>,
...
impl FsInputStreamWrapper {
    pub fn new(shared_vfs: Arc<Mutex<Fs>>, fd: u32, offset: u64) -> Self {
        Self {
            shared_vfs,
            fd,
            offset,
...
impl FsOutputStreamWrapper {
    pub fn new(shared_vfs: Arc<Mutex<Fs>>, fd: u32, offset: Option<u64>) -> Self {
        Self {
            shared_vfs,
            fd,
            offset,
...
impl HostInputStream for FsInputStreamWrapper {
    fn read(&mut self, size: usize) -> StreamResult<Bytes> {
        ...
        let mut vfs = self.shared_vfs.lock().unwrap();

        // Seek to offset
        vfs.seek(self.fd, offset as i64, SEEK_SET).map_err(|e| {
            StreamError::LastOperationFailed(anyhow::anyhow!("seek failed: {:?}", e))
        })?;

        // Read data
        let mut buf = vec![0u8; size];
        let n = vfs.read(self.fd, &mut buf).map_err(|e| {
    ...
    fn write(&mut self, bytes: Bytes) -> StreamResult<()> {
        ...
        match self.offset {
            Some(offset) => {
                ...
                let n = vfs.write(self.fd, &bytes).map_err(|e| {
            ...
            None => {
                ...
                vfs.append_write(self.fd, &bytes).map_err(|e| {
...
impl wasmtime_wasi::bindings::sync::filesystem::types::HostDescriptor for VfsHostState {
    ...
    > {
        ...
        // Seek to offset
        vfs.seek(fd, offset as i64, SEEK_SET)
    ...
    > {
        ...
        let vfs = self.shared_vfs.lock().unwrap();
        let meta = vfs.fstat(fd).map_err(convert_fs_error_to_trappable)?;
    ...
    > {
        ...
        let fd = {
            let mut vfs = self.shared_vfs.lock().unwrap();
            vfs.open_path_with_flags(&full_path, fs_flags)
                .map_err(convert_fs_error_to_trappable)?
        };
