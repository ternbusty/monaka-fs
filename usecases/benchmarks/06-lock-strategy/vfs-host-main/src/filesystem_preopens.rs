...
//
// Implements wasi:filesystem/preopens interface using main-branch fs-core with Mutex
...
impl wasmtime_wasi::bindings::sync::filesystem::preopens::Host for VfsHostState {
    fn get_directories(
        ...
        anyhow::Error,
    > {
        // Open root directory using fs-core (with Mutex lock)
        const O_RDONLY: u32 = 0;
        const O_DIRECTORY: u32 = 0x10000;

        let fd = {
            let mut vfs = self.shared_vfs.lock().unwrap();
            vfs.open_path_with_flags("/", O_RDONLY | O_DIRECTORY)
