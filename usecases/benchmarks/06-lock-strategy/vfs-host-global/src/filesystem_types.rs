// WASI Filesystem Types Host Implementation for fs-global
...
impl HostInputStream for FsInputStreamWrapper {
    fn read(&mut self, size: usize) -> StreamResult<Bytes> {
        ...
        self.shared_vfs
            .seek(self.fd, offset as i64, SEEK_SET as u32)
            .map_err(|e| {
...
impl wasmtime_wasi::bindings::sync::filesystem::types::HostDescriptor for VfsHostState {
    ...
    > {
        ...
        self.shared_vfs
            .seek(fd, offset as i64, SEEK_SET as u32)
    ...
    fn drop(
        ...
        let _ = self.shared_vfs.close(wrapper.fd);
        Ok(())
    }
