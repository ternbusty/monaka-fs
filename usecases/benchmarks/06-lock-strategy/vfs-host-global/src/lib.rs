// VFS Host Implementation using fs-global (global-lock)

use anyhow::Result;
use std::sync::Arc;

pub use fs_global::Fs;
...
/// Host state using fs-global (global-lock filesystem).
...
/// Helper function to convert fs-global error to WASI error code
pub fn convert_fs_error(
    error: fs_global::FsError,
) -> wasmtime_wasi::bindings::sync::filesystem::types::ErrorCode {
    use fs_global::FsError;
...
/// Add WASI interfaces to linker with global-lock VFS filesystem implementation.
