//! Benchmark for lock-none strategy (HashMap + UnsafeCell, no locking)
...
const STRATEGY_NAME: &str = "lock-none";
...
fn main() -> Result<()> {
    ...
    eprintln!();
    eprintln!("Strategy: {} (no locking - UNSAFE)", STRATEGY_NAME);
    eprintln!();
    ...
    if correctness.is_pass() {
        ...
        eprintln!(
            "WARN: Expected {} lines, found {} (errors: {}, strategy: {}) - Data loss expected with no locking",
